...
def run_sequential() -> tuple[list[str], float]:
    ...
    for i, prompt in enumerate(PROMPTS, 1):
        result = str(session.instruct(prompt + "\n Answer in maximum 1 sentence.", requirements=REQUIREMENTS))
...
async def instruct_and_resolve(session: MelleaSession, prompt: str) -> str:
    mot = await session.ainstruct(prompt, requirements=REQUIREMENTS)
...
async def run_parallel(method: str = "batched") -> tuple[list[str], float]:
    ...
    if method == "interleaved":
        ...
        mots = await asyncio.gather(*[session.ainstruct(p, requirements=REQUIREMENTS) for p in prompts])
...
def main():
    ...
    print("=" * 60)
    print("Batching comparison with requirement: 'Use a formal language.'")
    print("Sequential vs Parallel (50 prompts)")
    ...
    session = MelleaSession(make_backend())
    for i in range(3):
        str(session.instruct("Say hello.", requirements=REQUIREMENTS))
